...
# ========== 路径配置 ==========
DATA_DIR = pathlib.Path(r"E:/数据平台案例汇总/图像生成模块/data")
TMP_DIR  = pathlib.Path(r"E:/数据平台案例汇总/图像生成模块/generated_images")
...
# ==========================================================
def generate_images(req: dict) -> dict:
    """
    接收前端 JSON 解析后的 dict:
       {
         "imageTypes": [...],
    ...
    try:
        df = load_and_filter(req["dataSource"], req["downloadParams"])
    except Exception as e:
        # 任何读取/过滤失败，直接返回失败
        return {"success": False, "msg": str(e), "images": []}
    ...
    for t in req.get("imageTypes", []):
        try:
            ...
            # 某一张画坏了也别影响其它
            images.append({"type": t, "error": str(ex)})
