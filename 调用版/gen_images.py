...
# ========== 路径配置 ==========
# 指定数据目录的绝对路径
DATA_DIR = pathlib.Path("E:/数据平台案例汇总/图像生成模块/data")  # 根据你实际的文件夹路径调整
TMP_DIR = pathlib.Path("E:/数据平台案例汇总/图像生成模块/generated_images")
TMP_DIR.mkdir(exist_ok=True)  # 确保生成图像的目录存在
...
def main(req_json):
    ...
    try:
        # 尝试将请求的 JSON 字符串解析为 Python 字典对象
        print(f"Received JSON string: {req_json}")  # 打印原始接收的字符串

        # print("Attempting to parse JSON request...")
        req = json.loads(req_json)
        # req = json.loads(test_json)
        # print("Parsed request successfully:", req)
    except Exception as e:
        # 如果解析失败，输出错误信息并返回失败响应
        sys.stderr.write(f"Error parsing JSON: {str(e)}\n")
        print(json.dumps({"success": False, "images": []}, ensure_ascii=False))
        return

    # 初始化图像信息列表
    images = []
    try:
        # 根据请求中的 dataSource 和 downloadParams 过滤数据
        print("Filtering data based on provided parameters...")
        df = load_and_filter(req["dataSource"], req["downloadParams"])  # 目前的代码功能只有时间和文件上的选择，我把文件改改看
        print(f"Filtered data, number of rows: {len(df)}")
    except Exception as e:
        # 如果加载或过滤数据时出现错误，输出错误信息并返回失败响应
        sys.stderr.write(f"Error loading or filtering data: {str(e)}\n")
        print(json.dumps({"success": False, "images": []}, ensure_ascii=False))
        return

    # 遍历请求中的 imageTypes，生成对应的图像
    for t in req.get("imageTypes", []):
        try:
            print(f"Generating image for type: {t}...")

            # 调用 generate_one() 函数生成图像并获取绝对路径和说明文字，通过类别选择合适的特征
            local_path, caption = generate_one(
                t,
                req["category"],
                int(req["subOption"]),
                df.copy())

            print(f"Image generated: {local_path}, with caption: {caption}")

            # 将生成的图像信息（类型、路径和说明）添加到 images 列表
            images.append({"type": t, "localPath": local_path, "caption": caption})
        except Exception as ex:
            # 如果生成图像时发生错误，输出错误信息，但不影响其他图像的生成
            sys.stderr.write(f"Failed to generate image for {t}: {ex}\n")
